...
public class World {
    ...
    final static float WORLD_MIN_X = -14.0f;
    final static float WORLD_MAX_X =  14.0f;
    final static float WORLD_MIN_Z = -15.0f;
    ...
    private void generateInvaders() {
        for (int row = 0; row < 4; row++) {
            for (int column = 0; column < 8; column++) {
                Invader invader = new Invader(-WORLD_MAX_X / 2 + column * 2.0f, 0, WORLD_MIN_Z + row * 2.0f);
