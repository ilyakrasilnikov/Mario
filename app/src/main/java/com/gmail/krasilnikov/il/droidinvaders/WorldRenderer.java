...
import com.gmail.krasilnikov.il.framework.gl.LookAtCamera;
...
public class WorldRenderer {
    ...
    public WorldRenderer(GLGraphics glGraphics) {
        ...
        directionalLight.setDirection(-1.0f, -0.5f, 0.0f);
    ...
    private void renderInvaders(GL10 gl, List<Invader> invanders, float deltaTime) {
        invaderAngle += 45 * deltaTime;
        Assets.invaderTexture.bind();
        Assets.invaderModel.bind();
        int len = invanders.size();
        for (int i = 0; i < len; i++) {
            Invader invader = invanders.get(i);
